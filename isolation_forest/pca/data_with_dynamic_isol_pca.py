...
def create_sliding_windows(data, window_size=WINDOW_SIZE):
    num_samples, num_features = data.shape      # Get the number of samples and features
    num_windows = num_samples - window_size + 1 # Calculate how many windows we can create with the given window size
    
    # Create the windows by slicing the data and flattening each window into a 1D array
    windows = []
    for i in range(num_windows):
        window = data[i : i + window_size].flatten()    # Flattening - necessary for KNN input
        windows.append(window)
        
    return np.array(windows)

# Function to perform anomaly detection using isolation forest and return the anomaly scores
...
if __name__ == "__main__":
    try: 
        ...
        for train_file in train_files:
            ...
            train_windows = create_sliding_windows(scaled_train_data, window_size=10)
            test_windows = create_sliding_windows(scaled_test_data, window_size=10)
            
            # Print the shapes of the created windows to verify correctness
            ...
            # Fit the PCA on the training data and transform both the training and test data using the fitted PCA model
            train_input = pca.fit_transform(train_windows)
            test_input = pca.transform(test_windows)
            print(f"Shape of train input after PCA: {train_input.shape}")
            print(f"Shape of test input after PCA: {test_input.shape}")
            
            # Perform the Isolation Forest algorithm
            anomaly_scores = detect_anomalies_iforest(train_input, test_input, n_estimators=N_ESTIMATORS)
        ...
        with open(output_dir / "if_with_windows_dynamic.json", "w", encoding="utf-8") as f:
