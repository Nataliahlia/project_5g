...
# Function to build the LSTM Autoencoder model (Better for timeseries)
def build_lstm_autoencoder_model(input_shape):
    # Encoder: LSTM layers to compress the sequence
    ...
    # Decoder: LSTM layers to reconstruct the sequence
    ...
    # Create the autoencoder model with gradient clipping to prevent exploding gradients
    optimizer = keras.optimizers.Adam(learning_rate=0.0001, clipvalue=1.0)
...
# Function to perform Root Cause Analysis with flexible sample selection
def perform_rca(server_name, test_windows, x_test_pred, y_pred, test_mse_loss,  plots_dir, scaled_test_data, sample_idx=None, mode='first'):
    anomaly_indices = np.where(y_pred == 1)[0]
    
    if len(anomaly_indices) == 0:
        print("Δεν βρέθηκαν ανωμαλίες για RCA.")
        return []
    
    # Choice of which anomalies to analyze based on the mode
    if sample_idx is not None:
        # The user provided a specific sample index to analyze, so we will analyze only that one
        if sample_idx < 0 or sample_idx >= len(test_windows):
            print(f"ΠΡΟΣΟΧΗ: Το δείγμα {sample_idx} είναι εκτός ορίων [0, {len(test_windows)-1}]")
            return []
        if sample_idx not in anomaly_indices:
            print(f"ΠΡΟΣΟΧΗ: Το δείγμα {sample_idx} δεν ανιχνεύτηκε ως ανωμαλία, αλλά θα αναλυθεί.")
        indices_to_analyze = [sample_idx]
    elif mode == 'all':
        indices_to_analyze = anomaly_indices
        print(f"Θα αναλυθούν όλες οι {len(indices_to_analyze)} ανωμαλίες.")
    elif mode == 'first':
        indices_to_analyze = [int(anomaly_indices[0])]
    elif mode == 'worst':
        # Find the index of the anomaly with the highest reconstruction error (worst anomaly) and analyze only that one
        worst_idx = int(anomaly_indices[np.argmax(test_mse_loss[anomaly_indices])])
        indices_to_analyze = [worst_idx]
        print(f"Επιλέχθηκε η χειρότερη ανωμαλία (index {worst_idx}, error={test_mse_loss[worst_idx]:.4f}).")
    elif mode == 'random':
        # Find a random anomaly index from the detected anomalies and analyze only that one
        random_idx = int(np.random.choice(anomaly_indices))
        indices_to_analyze = [random_idx]
        print(f"Επιλέχθηκε τυχαία η ανωμαλία στο index {random_idx}.")
    else:
        print(f"Άγνωστο mode '{mode}'. Χρήση 'first'.")
        indices_to_analyze = [int(anomaly_indices[0])]
    
    # Perform RCA for the selected indices
    for rca_idx in indices_to_analyze:
        print(f"RCA για δείγμα στο index {rca_idx}...")
        analyze_anomaly(rca_idx, test_windows, x_test_pred, server_name, plots_dir, scaled_test_data=scaled_test_data)
    
    return list(indices_to_analyze)
...
if __name__ == "__main__":
    try: 
        ...
        output_dir = Path("results_lstm_10")
        ...
        for train_file in train_files:
            ...
            print("Κατασκευή LSTM Autoencoder μοντέλου...")
            input_shape = (train_windows.shape[1], train_windows.shape[2])
            model = build_lstm_autoencoder_model(input_shape)
            ...
            print(f"Χρόνος εκπαίδευσης: {train_time:.2f} sec")
            print(f"Epochs μέχρι σύγκλιση: {epochs_run}")
            ...
            print("Grid Search για καλύτερο threshold...")
            best_thresh, best_f1_pa, best_perc = find_best_threshold_grid_search(y_true, test_mse_loss)
            y_pred = (test_mse_loss > best_thresh).astype(int)
            metrics, metrics_pa = get_metrics_with_pa(y_true, y_pred)
            
            # Create and save plots for this server
            print("Δημιουργία γραφημάτων...")
            create_plots(server_name, test_mse_loss, y_true, y_pred, best_thresh, plots_dir)
            # Perform interactive RCA for the detected anomalies with flexible sample selection
            # Interactive RCA selection menu (user chooses which anomaly to analyze)
            print("\nΕπιλογές RCA:")
            print("  1) Πρώτη ανωμαλία")
            print("  2) Χειρότερη ανωμαλία")
            print("  3) Τυχαία ανωμαλία")
            print("  4) Όλες οι ανωμαλίες")
            print("  5) Δώσε index δείγματος (0-based)")
            choice = input("Επίλεξε (1-5, Enter=1): ").strip()
            if choice == "":
                choice = "1"
            ...
            if choice == "1":
                ...
                idx_str = input("Δώσε index δείγματος για ανάλυση (0-based): ").strip()
                try:
                    sample_idx = int(idx_str)
                except:
                    print("Μη έγκυρος αριθμός — θα αναλυθεί η πρώτη ανωμαλία.")
                    sample_idx = None
                    mode = "first"
            else:
                print("Άγνωστη επιλογή — χρήση 'first'.")
                mode = "first"

            print(f"Εκτέλεση RCA -> mode={mode}, sample_idx={sample_idx}")
            perform_rca(
            ...
            print(f"Βέλτιστο Threshold: {best_thresh:.6f}")
        ...
        print(f"{'='*80}")
        avg_time = np.mean([
            results_json["servers"][s]["training_time_sec"]
            for s in results_json["servers"]
        ])

        avg_epochs = np.mean([
            results_json["servers"][s]["epochs"]
            for s in results_json["servers"]
        ])

        print(f"Μέσος χρόνος εκπαίδευσης: {avg_time:.2f} sec")
        print(f"Μέσος αριθμός epochs: {avg_epochs:.2f}")
        ...
        results_json["overall"] = {
            "avg_training_time_sec": float(avg_time),
        ...
        with open(output_dir / "autoencoder_result_lstm_10.json", "w", encoding="utf-8") as f:
            json.dump(results_json, f, indent=4)
        print(f"\Τα αποτελέσματα αποθηκεύτηκαν στο: results/autoencoder_results_lstm_10.json")
